...
class Listing: NSObject, MKAnnotation{
    ...
    func deleteData(listing: Listing, completed: @escaping (Bool) -> ()){
        let db = Firestore.firestore()
        db.collection("listing").document(listing.documentID).delete()
            { error in
                if let error = error{
